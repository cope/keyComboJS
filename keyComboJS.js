...
cope.KeyCombo = ("undefined" === typeof cope.KeyCombo) ? {} : cope.KeyCombo;
cope.KeyCombo.Keys = ("undefined" === typeof cope.KeyCombo.Keys) ? {} : cope.KeyCombo.Keys;

cope.KeyCombo.Keys.ALT = 1;
cope.KeyCombo.Keys.CTRL = 2;
cope.KeyCombo.Keys.SHIFT = 3;

cope.KeyCombo.checkAlt = [];
cope.KeyCombo.checkCtrl = [];
cope.KeyCombo.checkShift = [];

cope.KeyCombo.alt = false;
cope.KeyCombo.ctrl = false;
cope.KeyCombo.shift = false;

cope.KeyCombo.combos = [];

cope.KeyCombo.addCombo = function (mainKey, keyCombo, callback) {
    if (!keyCombo) {
        return console.log("No keys defined.");
    }
    ...
    cope.KeyCombo.combos.forEach(function (combo) {
        if (mainKey === combo.mainKey && codes === combo.codes) {
            if (combo.callback && {}.toString.call(combo.callback) === '[object Function]') {
                ...
                if (mainKey === cope.KeyCombo.Keys.ALT) {
                    cope.KeyCombo.checkAlt = [];
                } else if (mainKey === cope.KeyCombo.Keys.CTRL) {
                    cope.KeyCombo.checkCtrl = [];
                } else if (mainKey === cope.KeyCombo.Keys.SHIFT) {
                    cope.KeyCombo.checkShift = [];
                }
            } else {
                console.log("No function defined for SHIFT combo: " + combo.keys);
            }
        }
    });
};
